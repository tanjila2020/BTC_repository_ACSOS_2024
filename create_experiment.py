...
def no_of_orders_list_creation(max_no_of_days, small_time_units, hourly_order_perc, periods_per_day, days_per_month, tot_order, min_order_perc, max_order_perc, demand_increase_dur, demand_increase_month_start, demand_increase_month, demand_increase_perc):
    ...
    # print(f"orders per day:{orders_for_days}") 
    ...
    # print(" hourly_orders_for_all_days",  hourly_orders_for_all_days)    
    ...
    # print(f"No_of_orders :{No_of_orders}")
...
max_no_of_days = 1###360 if we wanna run for 1 yr considering 30 days = 1 month
# hourly_order_perc = [0.01, 0.01, 0, 0, 0, 0, 0, 0.01, 0.02, 0.02, 0.03, 0.04, 0.05, 0.08, 0.09, 0.08, 0.07, 0.11, 0.13, 0.11, 0.09, 0.03, 0.02, 0.0]  ## actual order perc array for del order
...
min_order_perc = 0.95 ##this is to not having the same no of orders in every day
max_order_perc = 1.05

# hourly_order_perc = [0.01, 0.01, 0.01, 0.02, 0.02, 0.03, 0.04, 0.05, 0.08, 0.09, 0.08, 0.05, 0.1, 0.12, 0.11, 0.09, 0.05, 0.02, 0.01, 0.01, 0]
# hourly_order_perc = [0.03, 0.06, 0.03, 0.05, 0.03, 0.04, 0.05, 0.04, 0.05, 0.04, 0.03, 0.04, 0.06, 0.05, 0.03, 0.05, 0.06, 0.04, 0.03, 0.06, 0.05, 0.04, 0.02, 0.02] ###for uniform distribution

# hourly_order_perc = [0.1, 0.3, 0.5, 0.1, 0]
small_time_units = 20### to randomly generate times between hours for order arrival
periods_per_day = len(hourly_order_perc) * small_time_units  ###will be 1440 for 24hrs if 1 period = 1 min
no_of_days = -1
...
demand_increase_dur = 2*days_per_month  ### every 2 months increase task demand and include new robots
demand_increase_month_start = 0
demand_increase_month = demand_increase_month_start + demand_increase_dur-1 ##this -1 is bcz the array index is counted from 0 not 1
...
total_area = 800 ###meter square assuming each small cell is 1m sq. in area
task_min = 10   ###min valuation
task_max = 100 ##max valuation



task_deadline = 5 ###a fixed time in minutes after that if the task is not allocated it will be removed from the avail_task
max_cs_counter = 3 ### configurable max time to wait before finally decide to charge
###energy parameters
Ebat = 111.0 ### maximum battery capacity
highest_soc = round(0.8 * Ebat)# soc for recharging till this threshold
lowest_soc = round(0.20 * Ebat)# soc for start recharging at this threshold for DEG baseline
soc_threshold = 50 ##energy threshold used as α
period_length_sec = 60 ## this is just to simulate the time updation
energy_consumption_while_idle = round((3.514 * period_length_sec/3600),8)  ### amount of energy drawn while idle, measured in freq of 0.883GHz

q1 = 1  ###weight factor used as β1
q2 = 1 ## weight factor used as β2

#######create robots with newer and older batteries####
no_of_old_robs = 1   ### no of old robots to consider
old_soc_file = 'history_soc_trace_6.2%_deg.csv'
old_deg_min = 0.00001
old_deg_max = 0.1
k = 0
demand_to_meet = 1
robots = [Robot(i,k).__dict__ for i in range(R)]   ###create robot objects
charging_locations = [(random.randint(0, total_area), random.randint(0, total_area)) for _ in range(cs_loc)]
# charging_locations = [(15, 164), (273, 64)]
print("DONE!! ready for running tests")

charging_stations = []
for i in range(C):
    location_index = i % cs_loc  # Divide the charging stations among the locations
...
# Parameters
params = {'task_min': task_min, 'task_max': task_max, 'no_of_old_robs': no_of_old_robs, 'T': T, 'task_deadline': task_deadline, 'max_cs_counter': max_cs_counter, 'max_no_of_days': max_no_of_days,
          'periods_per_day': periods_per_day, 'tot_order': tot_order, 'max_order_per_k': max_order_per_k, 'demand_to_meet': demand_to_meet, 'R': R, 'C': C, 'cs_loc': cs_loc,
          'total_area': total_area, 'Ebat': Ebat, 'highest_soc': highest_soc, 'lowest_soc': lowest_soc, 'soc_threshold': soc_threshold, 'old_soc_file': old_soc_file, 'old_deg_min': old_deg_min, 'old_deg_max': old_deg_max,
          'q1': q1, 'q2': q2, 'period_length_sec': period_length_sec, 'energy_consumption_while_idle': energy_consumption_while_idle
          }
all_data = {
    "robots": robots,
